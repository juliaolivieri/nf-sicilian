...
def get_args():
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="bams to parse (either one or two for paired end)",
    )
    parser.add_argument("--outpath", help="folder to write output to")
    parser.add_argument(
        "--UMI_bar", action="store_true", help="extract UMI and barcode"
    )
    parser.add_argument(
        "--paired",
        action="store_true",
        help="run once with each read primary and concatenate the files",
    )
    parser.add_argument(
        "--annotator", required=True, help="the path to the annotator pickle file"
    )
    parser.add_argument("--gtf", required=True, help="the path to the gtf file")
    parser.add_argument(
        "--stranded_library",
        action="store_true",
        help="Take gene strand information directly from the read strand",
    )
...
def extract_info_align(
    ...
    empty_cols = [
        "aScore{}B".format(suffix),
        "qual{}B".format(suffix),
        "NH{}B".format(suffix),
        "nmm{}B".format(suffix),
        "HI{}B".format(suffix),
    ]
...
def extract_info_chim(
    ...
    refName, chrA, geneA, posA, chrB, geneB, posB = chim_refName(
        [x.flag for x in reads],
        [x.cigarstring for x in reads],
        [x.reference_start + 1 for x in reads],
        [bam_file.get_reference_name(x.tid) for x in reads],
        ann,
        stranded_library,
    )
    CI_dict["refName_AB" + suffix].append(refName)
    ...
    for i in range(2):
        ...
        CI_dict["read_strand{}{}".format(suffix, halves[i])].append(
            read_strand(reads[i].flag)
        )
        CI_dict["flag{}{}".format(suffix, halves[i])].append(reads[i].flag)

        CI_dict["primary{}{}".format(suffix, halves[i])].append(
            sec_dict[reads[i].is_secondary]
        )
    return CI_dict
    ...
    for i in range(len(bam_files)):
        ...
        for bam_read in alignFile.fetch(until_eof=True):
            ...
            if not bam_read.is_unmapped:
                if (i == 0) or (
                    not bam_read.is_secondary and bam_read.query_name in read_ids
                ):
...
#                            )
                        ...
                        # save genomic alignment information
                            ...
                            if i == 0:
                                if bam_read.query_name not in genomic_alignments:
                                    ...
                                    genomic_alignments[bam_read.query_name] = max(
                                        bam_read.get_tag("AS"),
                                        genomic_alignments[bam_read.query_name],
                                    )
                            else:
                                CI_dict = extract_info_align(
                                    CI_dict,
    ...
    if len(bam_files) == 2:
        final_df = pd.merge(
            left=CI_dfs[0],
            right=CI_dfs[1][
                [c for c in CI_dfs[1].columns if c not in ["UMI", "barcode", "seqR2"]]
            ],
            how="left",
            left_on="id",
            right_on="id",
        ...
        final_df.loc[
            final_df["read_strandR1A"] == final_df["read_strandR2A"],
    ...
    final_df["max_id_priority"] = final_df["id"].map(
        final_df.groupby("id")["HIR1A"].min()
    )
...
def main():
    ...
    for j in range(n_rounds):
        if j == 1:
            bam_files.reverse()
        print("bam_files", bam_files)
        primary = get_final_df(
            bam_files, j, suffixes, ann, args.UMI_bar, t0, gtf, args.stranded_library
        )
    ...
    pd.concat(final_dfs, axis=0).reset_index(drop=True).to_parquet(
        args.outpath + "class_input.pq"
    )
    ...
    pd.concat(final_dfs, axis=0).reset_index(drop=True).to_csv(
        args.outpath + "class_input.tsv", sep="\t", index=False
    )
