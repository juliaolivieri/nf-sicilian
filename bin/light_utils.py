...
def modify_refnames(CI, gtf_file, stranded_library):
    ...
    CI_new["junc_type"] = ""
    ind = CI_new[
        (CI_new["chrR1A"] != CI_new["chrR1B"])
        | (
            (CI_new["read_strandR1A"] == CI_new["read_strandR1B"])
            & (abs(CI_new["juncPosR1A"] - CI_new["juncPosR1B"]) >= 1000000)
        )
        & (CI_new["fileTypeR1"] == "Chimeric")
    ].index
    ...
    ind = CI_new[
        (CI_new["junc_type"] != "sc")
        & (CI_new["junc_type"] != "fus")
        & (
            (
                (CI_new["read_strandR1A"] == "+")
                & (CI_new["juncPosR1A"] > CI_new["juncPosR1B"])
            )
            | (
                (CI_new["read_strandR1A"] == "-")
...
def get_loc_flag(row):
    ...
    if "|rev" in row["refName_ABR1"]:
        if (row["juncPosR1A"] <= row["juncPosR2A"] <= row["juncPosR1B"]) or (
            row["juncPosR1B"] <= row["juncPosR2A"] <= row["juncPosR1A"]
        ):
            return 1
        else:
            return 0
...
def chim_refName(flags, cigars, offsets, rnames, ann, stranded_library):
    ...
    gene1, strand1 = ann.get_name_given_locus(
        rnames[0], posFirst, signs[0], stranded_library
    )
    gene2, strand2 = ann.get_name_given_locus(
        rnames[1], posSecond, signs[1], stranded_library
    )
    ...
    if rnames[0] != rnames[1]:
        ...
        juncType = "sc"
    elif (signs[0] == "+" and posFirst > posSecond) or (
        signs[0] == "-" and posFirst < posSecond
    ):
        juncType = "rev"
    elif (signs[0] == "+" and posFirst < posSecond) or (
        signs[0] == "-" and posFirst > posSecond
...
def readObj_refname(flag, cigar, seqname, position, ann, fill_char, stranded_library):
    ...
    if "N" not in cigar:
        ...
        gene, strand = ann.get_name_given_locus(
            seqname, int(position), read_strand, stranded_library
        )
        return (
            "{}:{}:{}".format(seqname, gene, strand),
            seqname,
            gene,
            position,
            fill_char,
    ...
    offset1 -= 1
    gene1, strand1 = ann.get_name_given_locus(
        seqname, offset1, read_strand, stranded_library
    )
    gene2, strand2 = ann.get_name_given_locus(
        seqname, offset2, read_strand, stranded_library
    )
